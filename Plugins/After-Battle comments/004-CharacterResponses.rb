...
def characterResponses(character,flags)
  ...
  mvp_5=mvps[4].name if !mvps[4].nil?
  ...
  lvp_5=lvps[4].name if !lvps[4].nil?
  ...
  if(defined?($NPCReactions[character.upcase][flag]))
    ...
    while i<$NPCReactions[character.upcase][flag].length
      ...
      message.gsub! '#{mvp_5}',mvp_5 if !mvp_5.nil?
      ...
      message.gsub! '#{lvp_5}',lvp_5 if !lvp_5.nil?
      message.gsub! '#{old_mvp_1}',old_mvp_1 if !old_mvp_1.nil?
      message.gsub! '#{type}',(GameData::Type.get(type).name) if !type.nil?
